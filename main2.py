...
from config import *
from dribble import release_ball_downward
from shoot import *
from utils import *
...
while True:
        ...
        if state["holding"]:
            ...
            if state["hold_hand"] in ["left","right"]:
                ...
                if hold_time > MIN_HOLD_BEFORE_RELEASE and speed_y > DRIBBLE_SPEED_THRESHOLD:
                    ...
                    is_behind = (state["hold_hand"]=="right" and right_behind) or (state["hold_hand"]=="left" and left_behind)
                    if is_low:
    ...
    """q# --- Unified main action status ---
    if state["last_label"]:
        # e.g. "Cross", "Behind Back", "Real Shot", "Fake Shot"
        status_main = state["last_label"].upper()
    else:
        # fallback to generic states
        status_main = "HOLDING" if state["holding"] else "DRIBBLING"

    hand_side = f"({state['hold_hand'] or 'auto'})"
    status_text = f"{status_main} {hand_side}"

    # draw single white line
